...
## [0.9.0] - 2026-08-16
### Added
- Server-side vocab enrichment integration. A new "Enrich vocab" button on the Vocabulary tab sends the active profile's vocab list to a configurable enrichment API, which returns dictionary-quality translations plus expanded grammatical inflections (`forms`) to increase in-page substitution coverage.
- A confirmation dialog is shown every time before submitting an enrichment request, since the action contacts an external server, consumes API key usage, and can end in overwriting local vocab — never triggered by a single accidental click.
- New `enrichment-api.js` module — the sole place that talks to the enrichment server. Every request resolves to a semantic `{ kind, ...}` result (`ok`, `pending`, `done`, `unauthorized`, `not_found`, `unsupported_pair`, `job_error`, `invalid_request`, `server_error`, `network_error`) rather than exposing raw HTTP status codes to the rest of the extension; network-level failures are caught internally and normalized the same way, so no other file needs try/catch around these calls.
- Per-profile API key storage (`profile.apiKey`), entered once via a modal on first use and only re-prompted reactively if the server reports it invalid — never a persistent, always-visible key-management control.
- Asynchronous job tracking per profile (`pendingEnrichJobId`, `pendingEnrichResult`, `lastEnrichStatus`), including a status bar (visible only on the Vocabulary tab, only for the profile that owns the job) showing live server-reported phase text. Terminal outcomes (done, or any failure) are persisted with an embedded timestamp, so a completed enrichment — or the last known failure — survives a dashboard refresh with zero further server contact once a result has been downloaded.
- A distinct enrichment-results diff modal (separate from the existing manual-import diff modal) showing translation-bank expansion rate, average translations per word (old → new), net translations added, and new/modified/removed row counts — computed against the *current* in-memory vocab so edits made after submitting an enrichment job aren't silently discarded. Declining the merge keeps the downloaded result cached for a later decision, with no re-fetch needed.
- Dashboard tab selection now persists across a page refresh (via URL hash), instead of always resetting to the Analytics tab.
- New shared `vocab-header-tokens.js`, consolidating the vocab-CSV header-recognition token list that was previously duplicated (and slowly drifting out of sync) between `content.js` and `vocab-import.js`.
- `manifest.json`: added `host_permissions` for the enrichment API's base URL (dev + eventual Render host).

### Changed
- `AhoCorasick.addEntry()` (in `aho-corasick.js`) once again falls back to `entry.translations` when `entry.forms` is empty, reverting the 0.6.5 forms-only tightening. Server-side enrichment (which populates `forms`) is now optional rather than assumed, so plain 2-column (`target`/`translations`) vocab lists remain fully functional for substitution without ever being enriched.
- Profile export (`exportProfile()`) now strips `apiKey`, `pendingEnrichJobId`, `pendingEnrichResult`, and `lastEnrichStatus` before serializing. Exporting/sharing a profile could otherwise leak the owner's API key, and cached job state is meaningless outside the key that created it.
- Enrichment failure states are shown as plain informational text with no dismiss/retry button, except where one is structurally necessary: `unauthorized` (must re-enter a key) and `done` (must explicitly pull the result). Every other failure state resolves on its own the next time "Enrich vocab" is clicked again, which always supersedes prior job tracking regardless of whether the new attempt succeeds or fails.
- Vocab csv generated by `duolingo-downloaded.js` column name changed from `[target, duolingo_translations]` to `[target, translations]`. Consistent with vocab parser and `aho-corasic.js`

## [0.8.3] - 2026-07-15
### Added
- Standalone Duolingo vocab downloader (`duolingo-downloader.js`), loaded as an isolated content script matching `https://www.duolingo.com/practice-hub/words*` only. Waits for the word-list DOM to appear, then prompts via `confirm()` to load all words (auto-expanding nested lists by repeatedly clicking trailing `li[role="button"]` elements), then prompts a second time with the loaded word count to confirm CSV export. Detected pairs (`li` containing `div > h3` + `div > p`) are saved as `duolingo_vocabulary_<timestamp>.csv` (semicolon-separated, `target;translations` header, quoted where needed).
